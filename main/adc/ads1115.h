...
public:
  bool begin(i2c_port_t port_num, uint8_t i2c_addr = ADS1X15_ADDRESS);
  ESP_ADS1X15::ERROR readADC_SingleEnded_Blocking(uint8_t channel, int16_t * value);


  
  ESP_ADS1X15::ERROR getLastConversionResults(int16_t *value);
  int32_t computeVolts(int16_t counts);
  void setGain(adsGain_t gain);
  adsGain_t getGain();
  void setDataRate(uint16_t rate);
  uint16_t getDataRate();

  ESP_ADS1X15::ERROR startADCReading(uint16_t mux, bool continuous);

  bool conversionComplete();

private:
  ESP_ADS1X15::ERROR writeRegister(uint8_t reg, uint16_t value);
  ESP_ADS1X15::ERROR readRegister(uint8_t reg, uint16_t *value);
  uint8_t buffer[3];
};
