...
void ble_init()
{
    ...
    esp_bluedroid_config_t blue_conf = BT_BLUEDROID_INIT_CONFIG_DEFAULT();
    ret = esp_bluedroid_init_with_cfg(&blue_conf);
    if (ret)
    {
...
size_t copy_can_frame_to_rc_frame(ble_can_frame_t *frame, uint8_t *rc_data)
{
...
bool ble_notify(ble_can_frame_t *frame)
{
